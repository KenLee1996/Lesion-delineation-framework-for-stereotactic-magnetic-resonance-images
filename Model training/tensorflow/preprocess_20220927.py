...
def scale_sliding_inference(img, model, inplane_size, slice_num, scale_factor, channel_num):
    stride = 64
    step = int((img.shape[1]-inplane_size)/stride)
    cer = np.zeros((int(img.shape[1]/scale_factor), int(img.shape[2]/scale_factor), img.shape[3], channel_num))
    pre = np.zeros_like(cer)
        ...
        for si in range(slice_step+1):
            ...
            for ix in range(step+1):
                ...
                for iy in range(step+1):
                    ...
                    scale_startx = int(startx/scale_factor)
                    scale_endx = int(endx/scale_factor)
                    scale_starty = int(starty/scale_factor)
                    scale_endy = int(endy/scale_factor)

                    cer[scale_startx:scale_endx,
                        scale_starty:scale_endy,
