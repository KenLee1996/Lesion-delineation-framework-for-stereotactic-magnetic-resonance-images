...
def process(data_dir, biparametric=False, slice_num=32):
    #print(data_dir)    
...
def tprocess(data_dir, biparametric=False, slice_num=32):
    #print(data_dir)
    x = np.load(data_dir)['data']    
    y = np.load(data_dir.replace('images','labels'))['data']
    x[np.where(np.isnan(x))] = 0
...
class Dataset_out(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_axc = nib.load(self.list_[index]+'/'+fn_axc)
        img_axc = img_axc.get_fdata()
        img_axc = np.rot90(img_axc, 1)
        img_axc = zscore_normalization(img_axc)
        if os.path.exists(self.list_[index]+'/'+fn_t2):
            img_t2 = nib.load(self.list_[index]+'/'+fn_t2)
            img_t2 = img_t2.get_fdata()
            img_t2 = np.rot90(img_t2, 1)
            img_t2 = zscore_normalization(img_t2)
        else:
            img_t2 = np.zeros(img_axc.shape)
            
        img = np.append(img_axc.reshape(img_axc.shape[0],img_axc.shape[1],img_axc.shape[2],1),
                        img_t2.reshape(img_axc.shape[0],img_axc.shape[1],img_axc.shape[2],1),
                        axis=3)
