...
class MyNumpyLoader(MultiFieldTransformer):
    ...
    def transform(self, transform_ctx: TransformContext):
        ...
        for field in self.fields:            
            ...
            transform_ctx.set_image(field, self._reader.read(file_name, flip_ratio, rot_angle, start0, start1, self._shape))
...
class MyNumpyReader(object):
    ...
    def read(self, file_name, flip_ratio, rot_angle, start0, start1, shape: ShapeFormat):
        ...
        block_size = 176 
        slice_num = 64
        if data.shape[2]>slice_num:
            start_slice = np.random.randint(0,data.shape[2]-slice_num)
            data = np.array(data)[start0:start0+block_size,start1:start1+block_size,start_slice:start_slice+slice_num]
        else:           
        ...
        if 'label' in file_name:
            #print('label',rot_angle,flip_ratio)
            data[data>=0.5] = 1
            data[data<0.5] = 0    
